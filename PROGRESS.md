...
| **Build / CI** | `lint` → `test` → `integration` (kind + digest-pinned node + `playtest-smoke.sh` with `CI=true`); optional **`git config core.hooksPath`** → tracked **`githooks/`** playtest; `make lint-docker` matches lint job. |
